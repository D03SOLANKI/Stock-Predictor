...
def test_entry_model(min_open_gap_pct=0.3, max_open_gap_pct=2.0, min_clean_air_threshold=4.0):
    capital = 100000.0
    fee = 0.0025
    trades = []
    for t in range(idx_test[0], idx_test[1]):
        ...
        # PURE 9:08 AM AUCTION RULES:
        open_gap = (o_T / pick['close_prev'] - 1.0) * 100.0
        
        # Must open positively with institutional bid (gap >= min_open_gap_pct)
        if open_gap < min_open_gap_pct: continue
        # Must not gap too high (exhaustion gap > max_open_gap_pct)
        if open_gap > max_open_gap_pct: continue
        
        # Pre-market Buy Stop order placed at trigger:
        if h_T < trigger: continue # Didn't trigger
        ...
        if hit_sl and hit_t1:
            pnl_gross = pick['sl_pct']; status = 'STOPPED_OUT_DUAL'
        elif hit_t2:
        ...
        capital *= (1.0 + pnl_net)
        trades.append({'date': date_T.strftime('%Y-%m-%d'), 'ticker': sym, 'pnl_net': round(pnl_net*100, 2), 'status': status, 'cap': round(capital, 2)})
...
print('=== PURE PRE-MARKET AUCTION BANDS (NO POST-9:08 AM KNOWLEDGE) ===')
for min_gap in [0.0, 0.2, 0.5, 0.8]:
    for max_gap in [2.0, 2.5]:
        res = test_entry_model(min_open_gap_pct=min_gap, max_open_gap_pct=max_gap)
        print('Gap Band [' + str(min_gap) + '% to ' + str(max_gap) + '%]: ' + str(res['n']) + ' trades | WR: ' + str(res['wr']) + '% | PF: ' + str(res['pf']) + ' | Ret: ' + str(res['ret']) + '% | Cap: Rs ' + str(res['cap']))
