...
print(f'Overall Universe: {res_all[\"n\"]} Trades | {res_all[\"wins\"]} Wins - {res_all[\"losses\"]} Losses | WR: {res_all[\"wr\"]}% | PF: {res_all[\"pf\"]} | Net Ret: {res_all[\"ret\"]:+.2f}%')
print(f'Mid Cap Only:     {res_mid[\"n\"]} Trades | {res_mid[\"wins\"]} Wins - {res_mid[\"losses\"]} Losses | WR: {res_mid[\"wr\"]}% | PF: {res_mid[\"pf\"]} | Net Ret: {res_mid[\"ret\"]:+.2f}%')
print(f'Small Cap Only:   {res_small[\"n\"]} Trades | {res_small[\"wins\"]} Wins - {res_small[\"losses\"]} Losses | WR: {res_small[\"wr\"]}% | PF: {res_small[\"pf\"]} | Net Ret: {res_small[\"ret\"]:+.2f}%')

print('\n' + '=' * 80)
print('PREDICTING TOP GAINER CANDIDATES FOR MID CAP & SMALL CAP')
print('=' * 80)

# Evaluate latest session (t = len(dates))
t_latest = len(dates)
...
mid_cands.sort(key=lambda x: x['score'], reverse=True)
small_cands.sort(key=lambda x: x['score'], reverse=True)

print(f'Mid Cap Qualified Candidates: {len(mid_cands)}')
for c in mid_cands[:3]:
    print(f\"  * {c['ticker']} ({c['name']}) | Score: {c['score']} | Trigger: Rs {c['trigger']} | SL: Rs {c['stop_loss']} (-2.2%) | T1: Rs {c['target_1']} (+3.5%) | T2: Rs {c['target_2']} (+6.5%)\")

print(f'\nSmall Cap Qualified Candidates: {len(small_cands)}')
for c in small_cands[:3]:
    print(f\"  * {c['ticker']} ({c['name']}) | Score: {c['score']} | Trigger: Rs {c['trigger']} | SL: Rs {c['stop_loss']} (-2.2%) | T1: Rs {c['target_1']} (+3.5%) | T2: Rs {c['target_2']} (+6.5%)\")
